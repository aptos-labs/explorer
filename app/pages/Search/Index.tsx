...
export default function SearchPage() {
  ...
  const runSearch = useCallback(
    async (searchText: string, signal: AbortSignal) => {
      ...
      try {
        ...
        // Fallback: check for owned objects if no results for address-like inputs
        if (
          filtered.length === 0 &&
        ...
        // Last resort: create a bare address link for valid addresses
        if (
          filtered.length === 0 &&
          (inputType.is32Hex || inputType.isValidAccountAddr)
        ) {
  ...
  // Debounce: update URL and fire search 400ms after the user stops typing
  useEffect(() => {
  ...
  }, [query, navigate, runSearch]);

  // On keyboard Enter in the input: navigate to the first result immediately
    ...
    <Stack spacing={3} sx={{maxWidth: 800, mx: "auto", pb: 6}}>
      ...
      {status === "idle" && !query && (
        <Typography variant="body2" color="text.secondary">
          ...
          <Box
            component="span"
            sx={{fontFamily: '"IBM Plex Mono", monospace', fontSize: "0.85em"}}
          >
            0x1
          </Box>
          {" · "}
