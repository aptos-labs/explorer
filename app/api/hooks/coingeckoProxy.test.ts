...
import {resolveCoingeckoUrl, shouldUseCoingeckoProxy} from "./coingeckoProxy";
...
describe("shouldUseCoingeckoProxy / resolveCoingeckoUrl", () => {
  ...
  it("uses the proxy when the cookie is exactly 'true'", () => {
    ...
    expect(resolveCoingeckoUrl("price", "/simple/price", "ids=aptos")).toBe(
      "/api/coingecko/price?ids=aptos",
    );
  });
  ...
  it("uses the direct upstream when the cookie is exactly 'false'", () => {
    ...
    expect(resolveCoingeckoUrl("price", "/simple/price", "ids=aptos")).toBe(
      "https://api.coingecko.com/api/v3/simple/price?ids=aptos",
    );
  });
