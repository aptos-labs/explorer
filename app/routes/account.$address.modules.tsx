...
// Layout route for /account/:address/modules/*
...
export const Route = createFileRoute("/account/$address/modules")({
  beforeLoad: ({params, search, location}) => {
    ...
    // Check if we're on the exact /account/:address/modules path (no child route)
    const pathSegments = location.pathname.split("/").filter(Boolean);
    const isExactMatch = pathSegments.length === 3; // ["account", "address", "modules"]
