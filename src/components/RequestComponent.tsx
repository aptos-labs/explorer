...
export function Loadable<T, E>({
  ...
  if (state.err) return React.cloneElement(children, {error: state.val});
  else return React.cloneElement(children, {data: state.val});
}
...
export function SafeRequestComponent<T, E = ResponseError>(
  props: RequestComponentProps<T, E>,
) {
  return (
    <ErrorBoundary>
...
export function RequestComponent<T, E = ResponseError>({
  ...
  return <Loadable state={loadingState}>{children}</Loadable>;
}
