...
const useProvideColorMode = () => {
  ...
  const [mode, setMode] = useState<Mode>((localStorage.getItem('color_scheme') as Mode) || prefersDarkMode)
  ...
  let theme = useMemo(() => responsiveFontSizes(createTheme(getDesignTokens(mode))), [mode])
