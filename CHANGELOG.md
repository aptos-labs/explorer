...
- **Netlify SSR deployment — `handler is not a function`**: The custom TanStack Start SSR entry now exports the documented `{ fetch }` server-entry shape required by Netlify's Vite adapter instead of a bare handler function, so the generated Netlify Function no longer resolves to an undefined default export at runtime. The `Accept: text/markdown` homepage response now runs in that fetch wrapper before TanStack's HTML-only SSR guard, keeping markdown negotiation out of Netlify Edge Functions. Netlify production builds also use the same Node major version as `.node-version` and CI (`24`) instead of the stale Node 20 build image pin. Regression tests now guard the SSR entry contract, markdown response path, Netlify build runtime, and absence of Edge Function configuration.
