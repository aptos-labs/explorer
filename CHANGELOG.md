...
- **pnpm 7-day minimum release age**: `.npmrc` now sets `minimum-release-age=10080` (minutes), so `pnpm install` will refuse to pick up any package version published in the last 7 days. This mitigates fast-moving supply-chain attacks (compromised maintainer tokens, malicious dependency releases) by giving the community time to detect and yank bad versions before they enter the explorer's dependency graph. Requires pnpm ≥ 10.16; the repo already pins `pnpm@10.33.0` via `packageManager`. Reproducible CI installs continue to use the existing `pnpm-lock.yaml`.
