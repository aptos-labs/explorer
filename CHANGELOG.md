...
- **E2E**: Playwright checks that mainnet requests to `api.mainnet.aptoslabs.com` send the embedded key in the `api-key` header (not `Authorization: Bearer AG-*`); skips when `VITE_APTOS_MAINNET_API_KEY` is unset
...
- **Aptos API Gateway (geomi) key usage**: embedded and user override `AG-*` keys are now sent with the `api-key` header instead of `Authorization: Bearer`, matching the Aptos Labs gateway so requests succeed with a key and usage appears in Geomi dashboards (Bearer was rejected with 401).
