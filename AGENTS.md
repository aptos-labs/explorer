...
- **Keep `"framework": "tanstack-start"` in `vercel.json`** and **`"buildCommand": "pnpm build"`**. Do **not** set `outputDirectory`. Root `index.html` is the Vite **dev** shell (`/app/client.tsx`). Nitro would copy that file into `.vercel/output/static` unless the client rollup input is `app/client.tsx`; Vercel’s filesystem handler would then serve it for `/`, browsers request `/app/client.tsx`, and the host returns `text/html` (MIME check fails). If the Vercel dashboard still has Framework **Vite** or Output Directory **`dist`** leftover from the import, change the preset to **TanStack Start** and clear the output directory so Nitro’s `.vercel/output` Build Output API is used.
