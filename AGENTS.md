...
**Keep i18n working in future changes.** Do not land new user-visible chrome, settings, search, or guide strings as hardcoded English. Add those keys to `app/i18n/messages/en.ts` first, then the same keys in every shipped catalog. Remaining explorer UI keys go in `app/i18n/messages/en/` (and the English barrel) and must be added to every **full-UI** locale (`FULL_UI_LOCALES`; currently every non-English shipped locale). A chrome-only catalog may omit those keys (English fallback). Use `useTranslation()` (`t` / `tList`) and locale-bound `formatNumber` / `formatInteger` / `formatDateTime` instead of ad-hoc `toLocaleString`. Language is selected from the header globe control (`LanguageSelect`, every viewport; shows `AUTO` while the preference is browser default, otherwise a short code for the current catalog) or the hamburger Language item (compact), and from `/settings`; all write immediately to `aptos-explorer-locale`. The picker lists **Browser default** (short code `AUTO`), then English, then other catalogs alphabetically by native name (`LANGUAGE_PICKER_LOCALES`, derived from `LOCALE_META`). Each catalog row shows the native name plus `localeShortLabel` (`EN`, `FR`, `简`, `BR`, `PT`). Do not add country flags to picker rows — languages are not countries, and several catalogs have no single accurate flag.
...
IDs must match `SUPPORTED_LOCALES` in `app/i18n/locales.ts` (registration order; append new catalogs). Native names match `LOCALE_META`. The in-app picker order is `LANGUAGE_PICKER_LOCALES`, not this table.
