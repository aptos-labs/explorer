...
| **Picker order** | **Browser default**, then English, then remaining catalogs alphabetically by native name (`LANGUAGE_PICKER_LOCALES`, `en` collation, base sensitivity). Shared endonym prefixes stay adjacent (Portuguese, Bahasa, Simplified/Traditional Chinese). Rows show native names, not country flags (languages are not countries; several catalogs have no single accurate flag). `SUPPORTED_LOCALES` stays registration order. |
...
| `app/components/layout/LanguageSelect.test.tsx` | FEAT-SETTINGS-003 / FEAT-CHROME-001 (header language control shows the current locale code, lists catalogs in `LANGUAGE_PICKER_LOCALES` order, persists `aptos-explorer-locale`, and restores the catalog after remount) |
| `app/i18n/locales.test.ts` | FEAT-SETTINGS-003 / FEAT-CHROME-001 (`localeShortLabel` compact codes for every shipped locale; `LANGUAGE_PICKER_LOCALES` is English-first then native-name alphabetical) |
