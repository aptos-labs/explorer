...
| **Scope** | Chrome (header, nav, footer, skip link, search placeholder/helper/type chips), settings copy, and the in-app user guide are translated in every shipped catalog. Remaining explorer UI copy (tabs, field labels, tables, errors, entity titles, and related chrome) lives in the English catalogs; `zh`, `fil`, `es`, `fr`, `de`, `ja`, `ko`, `pt`, `ar`, `vi`, `ha` include those keys, and other locales fall back to English. On-chain identifiers stay untranslated. |
...
| **Shipped locales** | Every registered catalog in `SUPPORTED_LOCALES` (see FEAT-SETTINGS-003) is available in the language picker. `zh`, `fil`, `es`, `fr`, `de`, `ja`, `ko`, `pt`, `ar`, `vi`, `ha` match the full English key tree. Other locales cover chrome, settings, search tokens, and the user guide; missing keys fall back to English. Automated tests enforce key/placeholder parity for the full-UI locales and subset/placeholder parity for the others. |
