...
| Aspect | Detail |
|--------|--------|
| **Links** | Privacy, Terms, Verification page, User Guide. |
...
| Aspect | Detail |
|--------|--------|
| **Behavior** | Global `defaultPendingComponent` shows a loading indicator during route transitions with minimum display time to reduce flicker. Account and transaction detail routes do **not** use a blocking route `pendingComponent`; they paint in-page skeletons immediately (FEAT-ACCOUNT-013, FEAT-TXN-015). |
...
| **Fallback** | Valid-looking address with no on-chain hits → still link to `/account/...` via `createFallbackAddressResult` (no extra owned-objects round trip). |
...
| **Multisig events** | Special `MultisigEventView` for `0x1::multisig_account::*` events (e.g. `TransactionExecutionSucceeded`, `TransactionExecutionFailed`, `ExecuteRejectedTransaction`, `CreateTransaction`, `Vote`, `AddOwners`, `RemoveOwners`, `UpdateSignaturesRequired`, `MetadataUpdated`). Renders a colored summary chip, labeled rows (addresses as `HashButton`s, vote as Approved/Rejected chip, execution error breakdown), a raw-JSON toggle, and surfaces unconfigured fields. Normalizes the v1 `*Event` suffix and the v2 module-event name to the same renderer. Unknown `0x1::multisig_account::*` event types fall back to the raw JSON view. |
| **Multisig payload decoding** | `decodeMultisigTransactionPayload` (`decodeMultisigPayload.ts`) BCS-decodes the stored payload bytes — the execution events' `transaction_payload` and a `CreateTransaction` event's inner `transaction.payload` — into the underlying entry function (function id linked to its Move source, type arguments, and arguments) with a "Decoded" chip and a toggle back to the encoded bytes. Uses the Aptos TS SDK (`MultiSigTransactionPayload` / `EntryFunction`); falls back to a raw hex byte view (with byte length) when the bytes are empty (payload-hash-only) or undecodable. `CreateTransaction` also shows the creator, creation time, and per-owner votes. |
| **Typed argument decoding** | `decodeMoveArgument` (`decodeMoveArgument.ts`) decodes each raw BCS argument into a typed value using the function's parameter types from the module ABI (fetched via `useGetAccountModule`, leading `signer`/`&signer` stripped). Supports `address` (→ account `HashButton`), `0x1::object::Object<…>` (→ object `HashButton`), `0x1::string::String`, `bool`, `u8`–`u256`, `vector<u8>` (hex bytes), nested `vector<…>`, and `0x1::option::Option<…>`; each value is labeled with a `MoveFunctionParamTypeBadge`. Unsupported types (custom structs, generics), undecodable bytes, or an unavailable ABI fall back to the raw argument bytes. |
| **Argument name resolution** | Decoded arguments and type arguments are labeled with their source names when resolvable, via the shared `useEntryFunctionArgNames` hook (manual override registry → Move source from the account's `PackageRegistry`, signer slots dropped by ABI position). Falls back to positional labels (`#0`, `T0`). The same hook backs the overview `TransactionArguments` view (FEAT-TXN-011). |
...
| Aspect | Detail |
|--------|--------|
| **Name resolution** | Function argument name overrides registry (see FEAT-DATA-003) → Move source extraction → positional fallback. Implemented by the shared `useEntryFunctionArgNames` hook (also used by the multisig payload decoder, FEAT-TXN-004). |
...
### FEAT-TXN-015 — Progressive Transaction Page Load
...
| Aspect | Detail |
|--------|--------|
| **Route** | `/txn/$txnHashOrVersion/payments`. User transactions only (placed after Overview / Decibel, before Balance Change). The tab is **omitted** when identification finds no payment steps (fees-only and unrecognized patterns). A `/payments` URL then rewrites to Overview. |
...
| **Empty** | The tab is not shown when no payment step is identified. If the panel still renders (tests, overridden tab lists), it explains "Network fees only" or "No payment identified". Failed user transactions still describe the intended payment, prefixed as failed. |
...
| Aspect | Detail |
|--------|--------|
| **Pruned history** | `/block/$height` (and by-version lookups) use serving fullnode REST first. The indexer does not expose a full REST-shaped block body, so a prune/404/401 falls through to the archive node without credentials (same URL discovery as FEAT-TXN-014). Recent-blocks list stays on the serving fullnode window. |
...
| Aspect | Detail |
|--------|--------|
| **Display** | Collapsible cards per Move resource type. Most resources use `JsonViewCard`. Structured summaries (with collapsible raw JSON): `0x1::decryption::PerEpochEncryptionKey`, `0x1::decryption::PerBlockDecryptionKey`, `0x1::confidential_asset::GlobalConfig`. |
...
| **Rewards earned** | `apt_rewards_distributed` is already denominated in APT. Rewards cells format the value with locale-aware decimal grouping (two fractional digits) and an `APT` suffix; they must not pass decimal strings through `APTCurrencyValue` with `decimals={0}` (integer octa path / `BigInt`). |
...
### FEAT-SETTINGS-003 — Language Preference

| Aspect | Detail |
|--------|--------|
| **Control** | A globe control with a short code (`LanguageSelect`) in the header toolbar next to the network selector on every viewport. The code is `AUTO` while the preference is browser default, otherwise the resolved catalog short code. Compact chrome (below `lg`, or `lg+` when translated labels overflow) also keeps a Language item in `HeaderOverflowMenu` (current native name as secondary text) that opens the same list. `/settings` keeps a labeled **Display language** select. All three apply immediately. Options: **Browser default** (`auto`, short code `AUTO`) or a registered catalog. Shipped catalogs: English (`en`), Simplified Chinese (`zh`), Traditional Chinese (`zh-Hant`), Filipino (`fil`), Spanish (`es`), French (`fr`), German (`de`), Japanese (`ja`), Korean (`ko`), Russian (`ru`), Brazilian Portuguese (`pt`), European Portuguese (`pt-PT`), Arabic (`ar`, RTL), Hindi (`hi`), Thai (`th`), Indonesian (`id`), Vietnamese (`vi`), Turkish (`tr`), Bengali (`bn`), Swahili (`sw`), Italian (`it`), Malay (`ms`), Tamil (`ta`), Ukrainian (`uk`), Dutch (`nl`), Polish (`pl`), Hebrew (`he`, RTL), Urdu (`ur`, RTL), Hausa (`ha`), Zulu (`zu`), and Amharic (`am`). |
| **Picker order** | **Browser default**, then English, then remaining catalogs alphabetically by native name (`LANGUAGE_PICKER_LOCALES`, `en` collation, base sensitivity). Shared endonym prefixes stay adjacent (Portuguese, Bahasa, Simplified/Traditional Chinese). Rows show the native name plus `localeShortLabel` (`EN`, `FR`, `简`, `BR`, `PT`), not country flags (languages are not countries; several catalogs have no single accurate flag). The Browser default row shows the same style of code: `AUTO`. The closed Settings control shows the native name only (Browser default stays the translated label, without `AUTO`). `SUPPORTED_LOCALES` stays registration order. |
| **Scope** | Chrome (header, nav, footer, skip link, search placeholder/helper/type chips), settings copy, the in-app user guide, and remaining explorer UI copy (tabs, field labels, tables, errors, entity titles, and related chrome) are translated in every shipped catalog. Remaining explorer UI copy lives in the English catalogs; every non-English shipped locale (`FULL_UI_LOCALES`) includes those keys. On-chain identifiers stay untranslated. Missing keys still fall back to English at runtime. |
...
| **Visible control** | Header Select on every viewport (desktop, compact, installed PWA). A status dot (mainnet/testnet/devnet/other) sits in the closed control so the current chain is scannable. Tooltip uses **Select Network** on the closed control only — it is hidden while the menu is open so it cannot cover **Mainnet**. |
...
| **Production HTML** | SSR from `app/routes/__root.tsx` (`<HeadContent />` for metadata, `<Scripts />` for hashed client assets). The repository has **no root `index.html`**, and `vite.config.ts` passes `renderer: false` to `nitro()` — otherwise Nitro adopts a root `index.html` as `renderer.template`, mounts `renderer-template` on `/**`, and never installs the `ssr-renderer` handler, so every response is that raw HTML file. `vercel.json` sets `"framework": "tanstack-start"` and `"buildCommand": "pnpm build"` with no `outputDirectory`, so Vercel consumes Nitro’s `.vercel/output` Build Output API. |
| **API catalog (RFC 9727)** | `public/.well-known/api-catalog` served as `application/linkset+json`. Lists upstream Aptos fullnode REST APIs (mainnet/testnet/devnet), **archive** REST APIs (mainnet/testnet), the indexer GraphQL API, and the explorer itself, with `service-desc`, `service-doc`, and `status` links. |
...
| **Trigger** | `app/components/layout/ShareButton.tsx` — `IosShareIcon` `IconButton` rendered in the persistent `Header` on every page when the explorer runs as an installed PWA **or** when the explorer is loaded inside an iframe. Hidden in regular top-level browser tabs (the browser already exposes a share/copy URL action there). No extra left margin — toolbar `columnGap` provides spacing so Share does not crowd network/language on a phone PWA. |
...
| **Header layout** | Network and language stay in the toolbar on every viewport so PWA users can switch chain/catalog without opening the hamburger. On wide viewports (`lg+`), the theme toggle is a dedicated toolbar `IconButton` (`ColorModeToggleButton`) beside Settings; on compact viewports it remains inside `HeaderOverflowMenu` — see FEAT-THEME-001. The AppBar pads `env(safe-area-inset-*)` and the root viewport uses `viewport-fit=cover` so those controls are not under the iOS notch/home indicator. |
...
### FEAT-I18N-001 — Message catalogs and locale resolution
...
**Route**: `/guide` (legacy `/help` redirects here, preserving `?network=`)

### FEAT-GUIDE-001 — In-app user guide
...
| `app/components/layout/LanguageSelect.test.tsx` | FEAT-SETTINGS-003 / FEAT-CHROME-001 (header language control shows the current locale code, lists catalogs in `LANGUAGE_PICKER_LOCALES` order with native name plus short code, persists `aptos-explorer-locale`, and restores the catalog after remount; Settings select lists the same rows and keeps the closed value as the native name) |
| `app/i18n/locales.test.ts` | FEAT-SETTINGS-003 / FEAT-CHROME-001 (`localeShortLabel` compact codes for every shipped locale; `localePickerRowLabel` joins native name and code; `LANGUAGE_PICKER_LOCALES` is English-first then native-name alphabetical) |
...
| `app/pages/Validators/Delegation/RewardsEarnedApt.test.ts` | FEAT-VALIDATORS-003 (`formatRewardsEarnedApt`: decimal APT rewards formatting, zero/rounding, locale separators; avoids `BigInt` on fractional rewards) |
...
| `e2e/smoke.spec.ts` | FEAT-GUIDE-001 / FEAT-CHROME-001 / FEAT-NETWORK-001 / FEAT-SETTINGS-003 (Playwright: `/guide` loads; document does not overflow horizontally on desktop and a 375px viewport; header language control is present on desktop and a 375px viewport next to the network selector; hamburger and network dropdown open on a 375px viewport without body scroll-lock; hamburger stays a 48px tap target) |
