...
| Aspect | Detail |
|--------|--------|
| **Control** | "Language" select on `/settings`: **Browser default** (`auto`) or a registered catalog (`en`, `zh`, `ko`, `ja`, `es`). |
...
| **Shipped locales** | `en` (English), `zh` (简体中文), `ko` (한국어), `ja` (日本語), `es` (Español). Missing keys fall back to English. |
