...
| **Shipped locales** | Every registered catalog in `SUPPORTED_LOCALES` (see FEAT-SETTINGS-003) is available in the language picker. Every non-English locale matches the full English key tree (`FULL_UI_LOCALES`). Missing keys fall back to English at runtime. Automated tests enforce key/placeholder parity for full-UI locales and subset/placeholder parity for any chrome-only locale. |
