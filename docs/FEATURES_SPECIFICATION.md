...
| **Multisig payload decoding** | `decodeMultisigTransactionPayload` (`decodeMultisigPayload.ts`) BCS-decodes the stored payload bytes — the execution events' `transaction_payload` and a `CreateTransaction` event's inner `transaction.payload` — into the underlying entry function (function id linked to its Move source, type arguments, and raw BCS argument bytes) with a "Decoded" chip and a toggle back to the encoded bytes. Uses the Aptos TS SDK (`MultiSigTransactionPayload` / `EntryFunction`); falls back to a raw hex byte view (with byte length) when the bytes are empty (payload-hash-only) or undecodable. `CreateTransaction` also shows the creator, creation time, and per-owner votes. |
