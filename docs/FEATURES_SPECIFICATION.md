...
| Aspect | Detail |
|--------|--------|
| **Control** | "Language" select on `/settings`: **Browser default** (`auto`) or a registered catalog (`en`, `zh`, `ko`, `ja`, `es`, `pt`, `vi`, `ar`). |
...
| **Shipped locales** | `en` (English), `zh` (简体中文), `ko` (한국어), `ja` (日本語), `es` (Español), `pt` (Português), `vi` (Tiếng Việt), `ar` (العربية). Missing keys fall back to English. |
